...
> **分类**：技能市场（第三方作品） ｜ **文件数**：13 ｜ **仓库目录**：`audio-meeting-transcript__skillhub`

## 📌 简介

录音转写技能——将音频转为带说话人识别和时间戳的录音稿，并生成会议纪要。类似飞书"妙记"。
...
```bash
# 用户级（推荐）
cp -r . ~/.workbuddy/skills/audio-meeting-transcript__skillhub

# 或项目级
cp -r . <你的项目>/.workbuddy/skills/audio-meeting-transcript__skillhub
```
